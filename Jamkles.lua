...
local ScreenGui = Instance.new("ScreenGui")
    ...
    TextButton.Text = "jamkles toggle"
...
ThemeManager:SetFolder('jamkleslua')
SaveManager:SetFolder('jamkleslua/configs')
...
local function createFOVCircle()
    ...
    circle.Thickness = 1
    circle.Color = jamkles.Triggerbot.FOV.Color
    circle.Filled = jamkles.Triggerbot.FOV.Filled
    circle.Visible = jamkles.Triggerbot.FOV.Enabled
    return circle
end
...
local function updateFOVCircle()
    if jamkles.Triggerbot.FOV.Enabled then
        FOVCircle.Radius = jamkles.Triggerbot.FOV.Size
        FOVCircle.Visible = true

        if jamkles.Triggerbot.FOV["Centered FOV"] then
...
local function getMouseTarget()
    local mousePos = jamkles.Triggerbot.FOV["Centered FOV"] 
                     and Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2) 
                     or UserInputService:GetMouseLocation()
    local closestPlayer = nil
    local closestDistance = jamkles.Triggerbot.FOV.Size

    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and isTargetValid(player) then
            for _, hitPartName in ipairs(jamkles.Triggerbot.ClosestPart.HitParts) do
...
local function fireAtTarget(target)
    if target then
        ...
        if Tool and Tool:FindFirstChild("Handle") then
            Tool:Activate()
            task.wait(jamkles.Triggerbot.Settings["Click Delay"])
            Tool:Deactivate()
        end
    end
end
    ...
    if AimlockState then
        ...
        if Locked then
            ...
            if Victim then
                ...
                if jamkles.targetaim.enabled then
                    jamkles.targetaim["Toggled"] = true
                ...
                if jamkles.targetaim.enabled then
                    jamkles.targetaim["Toggled"] = false
...
RS.RenderStepped:Connect(function()
    if AimlockState and Victim and Victim.Character and Victim.Character:FindFirstChild(jamkles.AimPart) then
        ...
        if jamkles.TargetStrafe.Enabled then
            ...
            if jamkles.TargetStrafe.RandomizerMode then
               
                strafeOffset = Vector3.new(
                    math.random(-jamkles.TargetStrafe.StrafeRadius, jamkles.TargetStrafe.StrafeRadius),
                    math.random(0, jamkles.TargetStrafe.StrafeHeight),
                    math.random(-jamkles.TargetStrafe.StrafeRadius, jamkles.TargetStrafe.StrafeRadius)
                )
            else
               
                strafeOffset = Vector3.new(
                    math.cos(tick() * jamkles.TargetStrafe.StrafeSpeed) * jamkles.TargetStrafe.StrafeRadius,
                    jamkles.TargetStrafe.StrafeHeight,
                    math.sin(tick() * jamkles.TargetStrafe.StrafeSpeed) * jamkles.TargetStrafe.StrafeRadius
                )
            end
...
mt.__namecall = newcclosure(function(Self, ...)
    ...
    if not checkcaller() and methodName == "FireServer" and jamkles.targetaim.enabled then
        for i, Argument in ipairs(args) do
            if typeof(Argument) == "Vector3" and target and target.Character then
                args[i] = target.Character[jamkles.targetaim.targetPart].Position + (target.Character[jamkles.targetaim.targetPart].Velocity * jamkles.targetaim.prediction)
...
jamkles.AntiGroundShots = true

if jamkles.AntiGroundShots then
    AimPoint = jamkles.AimPart.Position + Vector3.new(jamkles.AimPart.Velocity.X, (jamkles.AimPart.Velocity.Y * 0.5), jamkles.AimPart.Velocity.Z) * prediction
else
    AimPoint = jamkles.AimPart.Position + jamkles.AimPart.Velocity * prediction
end


while jamkles.AutoReload == true and game:GetService("RunService").Heartbeat:Wait() do
...
if jamkles.desync.sky == true then
...
if jamkles.desync.jump == true then
...
if jamkles.desync.network == true then
local RunService = game:GetService("RunService")
...
if jamkles.Misc.LowGfx == true then
game:GetService("CorePackages").Packages:Destroy()
end

if jamkles.FPSunlocker.Enabled then
    setfpscap(jamkles.FPSunlocker.FPSCap)
end

if jamkles.AntiAimViewer == true then

--// Services
local Jamkles = setmetatable({}, {
    __index = function(_, service)
        return game:GetService(service)
    end
})

local Players, ReplicatedStorage = Jamkles.Players, Jamkles.ReplicatedStorage
...
local function resolveTargetVelocity(target)
    if getgenv().jamkles.Resolver.Enabled then
        if getgenv().jamkles.Resolver.Method == "RecalculateVelocity" then
            ...
            if lastPosition then
                local resolvedVelocity = (target.Character[getgenv().jamkles.Resolver["Prediction Settings"].HitPart].Position - lastPosition) / deltaTime
                lastPosition = target.Character[getgenv().jamkles.Resolver["Prediction Settings"].HitPart].Position
    ...
    return target.Character[getgenv().jamkles.Resolver["PredictionSettings"].HitPart].Velocity
end
